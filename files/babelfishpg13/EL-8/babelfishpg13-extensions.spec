...
# Even though we incorporated the babelfishpg_NAME extension directories
# under ./contrib, we still have to build them as PGXS only modules because
# the fixes for the Makefiles have not been done yet. This means we need
# to set the environment variables expected by Babelfish.
PG_CONFIG=/usr/pgsql-%{pgmajorversion}/bin/pg_config
PG_SRC=%{_builddir}/%{sname}-engine-%{babelfishversion}-PG-%{pgversion}
cmake=/usr/bin/cmake
export PG_CONFIG PG_SRC cmake

# Compile the Babelfish extensions
%{__make} %{?_smp_mflags} -C contrib/babelfishpg_money all
%{__make} %{?_smp_mflags} -C contrib/babelfishpg_common all
%{__make} %{?_smp_mflags} -C contrib/babelfishpg_tds all

# For some reason the antlr lexer and parser parts aren't built
# under rpmbuild if we just call 'make -C ...' for babelfishpg_tsql.
# Workaround is to go in there and do it explicitly.
cd contrib/babelfishpg_tsql/antlr
ANTLR4_JAVA_BIN=/usr/bin/java
ANTLR_EXECUTABLE=$(pwd)/thirdparty/antlr/antlr-4.9.3-complete.jar
export ANTLR4_JAVA_BIN ANTLR_EXECUTABLE
cmake .
make
cd ../../..
%{__make} %{?_smp_mflags} -C contrib/babelfishpg_tsql || true
...
# Install the Babelfish Extensions. Again, we need to set environment
# variables to make the hardcoded PGXS Makefiles work.
PG_CONFIG=/usr/pgsql-%{pgmajorversion}/bin/pg_config
PG_SRC=%{_builddir}/%{sname}-engine-%{babelfishversion}-PG-%{pgversion}
cmake=/usr/bin/cmake
export PG_CONFIG PG_SRC cmake
